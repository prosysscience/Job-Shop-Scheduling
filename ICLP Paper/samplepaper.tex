...
\end{table}
%
In Table~\ref{tab:Table02}, we compare other problem decomposition strategies
to the static Machine-based EST approach, providing the baseline abbreviated by M-EST.
Further decomposition strategies are denoted by M-MTWR for the static Machine-based MTWR
variant, corresponding dynamic versions M-EST$^\text{\,d}$ and M-MTWR$^\text{\,d}$ that
determine EST and MTWR values w.r.t.\ partial schedules from previous time windows, as well as
static Job-based counterparts J-EST and J-MTWR in the first two rows.
...
based on the smallest EST or greatest MTWR value as secondary criterion.
