...
Starting with $H:=\emptyset$, for $w:= 1$ to number $n$ of time windows, do:
\begin{enumerate}[label=\textbf{\arabic*.}]
  ...
  \item While the time limit per time window is not reached, do:
  \begin{enumerate}[label=\textbf{\alph*.}]
  \item Find some answer set $X$
  \end{enumerate}
  \item Apply the compression of a window (postprocessing phase).
  \item Keep the operations assignment to be passed to the next window.
  \item Go to \textbf{2} until all windows are optimized.
\end{enumerate}
  \begin{enumerate}[label=\textbf{\arabic*.}]
    \item Ground the base program.
    \item Ground the next shot.
    \item Optimize the current window till reaching the optima or the timeout.
    \item Apply the compression of a window (postprocessing phase).
    \item Keep the operations assignment to be passed to the next window.
    \item Go to \textbf{2} until all windows are optimized.
  \end{enumerate}
