...
\usepackage{soul}%{color,soul}
\usepackage{pagecolor}
\sethlcolor{\thepagecolor}
...
%
\begin{abstract}
Scheduling methods are important for effective production and logistics management, where tasks need to be allocated and performed with limited resources.
In particular, the Job-shop Scheduling Problem (JSP) is a well-known and challenging combinatorial optimization problem in which tasks sharing a machine are to be arranged in a sequence such that encompassing jobs can be completed as early as possible.
% remove double 'so that'
Given that already moderately sized JSP instances can be highly combinatorial, and neither optimal schedules nor the runtime to termination of complete optimization methods is known, efficient approaches to approximate good-quality schedules are of interest.
In this paper, we propose problem decomposition into time windows whose operations can be successively scheduled and optimized by means of multi-shot Answer Set Programming (ASP) solving.
From a computational perspective, decomposition aims to split highly complex scheduling tasks into better manageable subproblems with a balanced number of  operations so that good-quality or even optimal partial solutions can be
...
in similar runtime on the entire instance.
...
into the iterative scheduling process to counteract window-wise optimization limitations restricted to partial schedules.
...
While the obtained solution quality still remains behind a state-of-the-art Constraint
Programming system, our multi-shot solving approach comes closer the larger the instance
size, demonstrating good scalability by problem decomposition.
%
...
Effective scheduling methods are essential for complex manufacturing and transportation systems, where allocating and performing diverse tasks within resource capacity limits is one of the most critical challenges for production management \citep{uzsoy2000performance}.
...
Finding optimal JSP solutions, determined by a sequence of operations for each machine, is an NP-hard combinatorial problem \citep{garey1976complexity,lenstra1977complexity,liu2008prediction}. Therefore, optimal schedules and termination guarantees can be extremely challenging or even unreachable for complete optimization methods already for moderately sized instances.
For example, it took about $20$ years to develop a search procedure able to find a (provably) optimal solution for an instance called FT10 with $10$ jobs
\citep{adams1988shifting,zhang2010hybrid}, each consisting of a sequence of $10$ operations to be processed by $10$ machines. 
...
where no single strategy strictly dominates \hl{in minimizing the tardiness} \citep{ovacik2012decomposition}. % \hl{Most of the previous work has applied metaheuristic techniques or local search algorithms for solving the problem without sharing their implementation.}
...
Successful \hl{applications in} areas beyond JSP include 
...
\item We experimentally evaluate decomposition strategies varying the number of
      ...
      where the gap increases with the number of operations to schedule.
      While the state-of-the-art CP system OR-tools \citep{ortools} is still ahead regarding the solution quality, our multi-shot solving approach % based on problem decomposition
...
\lstinputlisting[float=b,label=prg:facts,caption={Example JSP instance},linerange={1-3},numbers=none,xleftmargin=3pt]{listing/facts.lp}
%
An atom of the form
...
within jobs is respected}. % , which respect the operation precedence within jobs and
% in order to enable a successive extension of good-quality partial schedules to a global solution.
...
\lstinputlisting[float=b,label=prg:decomposition,caption={J-EST decomposition encoding}]{listing/decomposition.lp}
% Only 
...
%
\lstinputlisting[float=b,label=prg:tw,caption={Example time windows},numbers=none,xleftmargin=3pt]{listing/TW.lp}
...
In single-shot ASP modulo DL solving with \clingodl, the minimization can be accomplished via the command-line option \lstinline{--minimize-variable=makespan}.
...
some (partial) schedule $X$ is feasible.
...
\begin{figure}[tb]
  ...
  \end{figure}%
  % \captionof{figure}{Control loop for successive schedule optimization by multi-shot ASP modulo DL solving\label{fig:enum}}
%
Once some schedule $X$ with a horizon $h+1$ is found in step~4a,
the step~4b consists of instantiating
the subprogram \lstinline{optimize(}$h$\lstinline{)} % is instantiated
on demand, i.e.,
...
a global solution 
% is implemented by means of the Python interface of \clingodl,
% where a time limit 
...
% and excluded the small instances because they could be solved by the existing methods and applying decomposition leads to unnecessary overheads.}
...
% KS removed, since it was already explained in Section 3.2
%The latter augments the encoding given in Listing~\ref{prg:decomposition} with bottleneck machines for inserting their operations into time windows, 
...
%    \begin{minipage}{\textwidth}
  ...
  \topline%\hline
%       & {}        & \multicolumn{6}{c}{Group of Instances}  \\
%  &  &  \multicolumn{7}{c}{Number of Time Windows} \\ [2pt] \cline{3-9} 
...
\end{table}
%
In Table~\ref{tab:Table02}, we provide a comparison of the Job- and Machine-based EST and MTWR approaches with %, providing the baseline abbreviated by M-EST.
%Further decomposition strategies are denoted by M-MTWR for the static Machine-based MTWR variant, 
...
% \ksverify{We excluded the dynamic variants of Job-based strategies, since they performed worse than their static originals.}
%, as well as static Job-based counterparts J-EST and J-MTWR in the first two rows.
...
%     \hline\\[-2.75mm]
    \midline
    0\%              & $3010.3$  & $907.5$   & $3.4$	   & $3174.5$      & $839.6$ & $3.0$ & $5671.9$ & $1000.0$ & $7.0$\\
    % [1.5mm]
...
\begin{table}[b]
  ...
  \topline%\hline
%  \multirow{3}{*}{Size} & \multicolumn{4}{c}{Makespan} & \multicolumn{3}{c}{Distance measures} \\
%  & Optima & OR-tools & \multicolumn{2}{c}{ASP} & OR-tools & \multicolumn{2}{c}{ASP} \\

 % & & & Single-shot & Multi-shot &  & Single-shot & Multi-shot 
 & & \multicolumn{2}{c}{OR-tools} & \multicolumn{2}{c}{Multi-shot} & \multicolumn{2}{c}{Single-shot} \\
...
we also ran the state-of-the-art CP system OR-tools (version 9.2)}\hl{
...
% \hl{We conducted other experiments to assess the quality of the solutions obtained by our proposed method by comparing ASP with single-shot and OR-tools (version 9.2)} \citep{ortools}\hl{. Table}~\ref{tab:Table04} \hl{summarizes the results of our experiments. The values in the table represent the average distance to the optimal solutions for each set of instances. The first column represents the size of the instances with the decomposition parameters. The second and third columns show the average distance of the results obtained by ASP single and multi-shot, respectively. The last column shows the average distance obtained by OR tools. The results show that the single-shot ASP is far from the optima, especially with the largest dataset. In addition, it is shown that the developed model improves the results compared to the pure ASP. However, the performance of OR tools is better than ours, especially for the first two sets of instances. For $100 \times 20$, our proposed model is a bit comparable to OR tools, which means that if the instance size increases, it could be possible to obtain better results with our approach.}
  
\begin{comment}
  \begin{table}[t]
  \caption{Experiments comparing the Single, Multi-shot ASP and OR tools\label{tab:Table04}}%
  \centering
  \begin{tabular}{l l r r }

  \topline%\hline
  %Timeout & \multicolumn{2}{c}{$1000$ sec} & \multicolumn{2}{c}{$500$ sec} \\
   & \multicolumn{2}{c}{ASP} & OR tools \\
  Instance set & Single-shot & Multi-shot &
  \midline

  $50\times15$  & $768.3$ & $169.3$ & $5.0$ \\

  %$50\times15$  & $169.3$ & $5.0$ & $163.3$ & $26.0$ \\
  ($4$w, M-EST, 30\%) & & & \\
  
  $50\times20$  & $1035.5$ & $303.8$ & $70.4$ \\
  %$50\times20$  & $303.8$ & $70.4$ & $304.4$ & $86.2$ \\
  ($4$w, M-EST, 20\%) & & & \\
  
  $100\times20$  & $41420.4$ & $253.5$ & $193.6$ \\
  %$100\times20$  & $253.5$ & $193.6$ & $286.3$ & $284.1$ \\
  ($9$w, M-EST, 20\%) & & &  
...
\hl{While Machine-based decomposition turned out to be helpful
for finding better schedules, the strategies presented in Section}~\ref{subsec:decomposition}
\hl{can be regarded as naive heuristics based on straightforward instance properties,
and incorporating machine-learning approaches} \citep{elscge22a,tagesc21a}
\hl{for more adept partitioning is another promising direction for future work.
Provided the adjustment of problem decomposition strategies, 
our multi-shot solving methods can also be applied to the 
Flow-shop and Open-shop Scheduling Problems} \citep{taillard1993benchmarks}.
...
\paragraph{Acknowledgments.}{This work was partially funded by
KWF project 28472,
cms electronics GmbH,
FunderMax GmbH,
Hirsch Armbänder GmbH,
