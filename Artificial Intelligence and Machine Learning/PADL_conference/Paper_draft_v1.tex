...
\label{sec:eval}
We use a subset of Taillard's benchmark suite \cite{taillard1993benchmarks}, comprising $10$ randomly generated JSP instances 
...
We utilized the above feature sets for partitioning the operations of JSP instances into
\todo{Please insert number of time windows.}$X$ clusters or time windows, respectively,
and then ran the multi-shot optimization approach introduced in~\cite{el2020job} for
$10$ minutes in total, i.e., \todo{Please insert minutes per time windows.}$X$ minutes per time window,
on an
\todo{Please check and update the machine and operating system setup.}
Intel\textsuperscript{\textregistered} Xeon\textsuperscript{\textregistered} E5520 machine under Linux (Ubuntu 18.04.3).
The resulting quality of schedules in terms of the achieved makespan for each instance is reported in Table~\ref{tab4},
...
clustering for the decomposition into \todo{Please insert number of time windows.}$X$ time windows,
for reference.
...
However, the feature sets \textbf{F6}, \textbf{F8}, \textbf{F11}, \textbf{F12}, and \textbf{F15}
